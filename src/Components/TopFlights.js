        ...
        <>
            ...
            {data.map((destination, index) => {
                return <div key={index} className='destination destination__vlc'>
                    ...
                    <p>Departure <strong>{destination.cityFrom}</strong>: {DateTime.fromMillis(destination.dTimeUTC * 1000).toFormat('HH:mm')} -     Arrival <strong>{destination.cityTo}</strong>: {DateTime.fromMillis(destination.aTimeUTC * 1000).toFormat('HH:mm')}</p>
